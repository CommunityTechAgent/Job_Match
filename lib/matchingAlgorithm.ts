...
export async function getActiveJobsForMatching(filters?: MatchFilters): Promise<any[]> {
  ...
  if (filters?.search) {
    ...
    if (searchTerm) {
      query = query.or(`title.fts.${searchTerm},description.fts.${searchTerm},requirements.fts.${searchTerm}`)
    }
  }
 ...
 */
function calculateBonusFactors(
  job: any,
  userProfile: Partial<Profile>,
): {
  score: number
  ...
  if (userProfile.preferred_salary_range && job.salary_range) {
    ...
    const salaryMatch =
      job.salary_range.toLowerCase().includes("competitive") || job.salary_range.toLowerCase().includes("market")
 ...
 */
export async function findJobMatches(userId: string, filters?: MatchFilters): Promise<MatchResult> {
  try {
    // Get user profile
    ...
    const scoredJobs: JobMatch[] = jobs.map((job) => {
      ...
      return {
        ...
        skills_overlap_percentage:
          matchingSkills.length > 0 ? (matchingSkills.length / (job.skills_required?.length || 1)) * 100 : 0,
    ...
    if (filters?.requiredSkills?.length) {
      filteredMatches = filteredMatches.filter((job) => {
        ...
        const matchingRequiredSkills = jobSkills.filter((skill: string) => requiredSkillsSet.has(skill.toLowerCase()))
 ...
 */
export async function getMatchRecommendations(userId: string, limit = 10): Promise<JobMatch[]> {
